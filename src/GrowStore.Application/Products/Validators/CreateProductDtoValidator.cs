...
public class CreateProductDtoValidator : AbstractValidator<CreateProductDto>
{
    public CreateProductDtoValidator()
    {
        ...
        RuleFor(x => x.Variants)
            .NotNull().WithMessage("Variants list is required.")
            .Must(variants => variants.Count > 0).WithMessage("At least one variant is required.")
            .ForEach(variant => variant.SetValidator(new CreateProductVariantDtoValidator()));
    }
}
